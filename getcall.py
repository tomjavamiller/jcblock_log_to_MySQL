...
engine = create_engine("mysql+pymysql://root:goopsoup@localhost/callerid?charset=utf8&use_unicode=0")
...
def queryB():
    ...
    query = "select number, name, count(*) from callerid c1 \
             where exists (select * from callerid c2 where c1.number = c2.number and c2.code = 'B' and \
                                                  c2.timestamp > DATE_SUB(NOW(), INTERVAL 20 DAY)) \
                   and not exists (select * from blocked b where c1.number = b.number and b.code is null) \
        group by number \
        having count(*) > 3 \
        order by count(*)"
...
def main():
    ...
    os_command = "scp pi@pi:/usr/local/jcblock/callerID.dat ."
    print(os_command)
